...
Le n\{oe}ud sinusal possède la propriété d'automatisme, elle se dépolarise à intervalles réguliers (définissant la période cardiaque) sans cause extérieure. Cependant, cette période cardiaque peut être modulée, principalement par l'action du système nerveux autonome (SNA). Les deux branches du SNA ont un effet antagoniste sur la fréquence cardiaque : le système parasympathique diminue la fréquence cardiaque et le système sympathique l'augmente. Le SNA agit aussi sur d'autres facteurs cardiaques : le système sympathique augmente la force de contraction, et les deux systèmes diminuent la période refractaire.


\subsection{Enregistrement électrocardiographique}

Comme le corps est électriquement conducteur, les différences de potentiel subies par chaque cellule se propagent dans toutes les directions et peuvent être enregistrées à distance à l'aide d'éléctrodes. L'ensemble de ces enregistrements forme l'électrocardiograme (ECG). Il existe deux modes d'utilisation des électrodes : bipolaire et unipolaire. Le mode bipolaire utilise deux électrodes placées à deux endroits et mesure la différence de potentiel entre ces deux zones. Au contraire, le mode unipolaire mesure la différence entre une électrode et un point de référence, qui peut être pris comme une mesure éloignée de la zone d'intérêt, ou une moyenne de plusieurs électrodes. Un enregistrement dans l'un de ces modes est appelé une dérivation de l'ECG.
...
\begin{enumerate}
\item trois dérivations bipolaires (DI, DII et DIII), en plaçant des électrodes sur le bras gauche, le bras droit et la jambe gauche, et en mesurant la différence entre les trois couples d'électrodes ;
...
\begin{figure}[h]
	...
	\caption{Schéma de l'emplacement des 12 dérivations classiques d'un ECG.}
	\label{ecg12}
\end{figure}

Au contraire de l'ECG douze dérivations, le Holter permet un enregistrement ambulatoire de longue durée sur au moins 24 heures. Il est constitué le plus souvent de 5 ou 7 électrodes, permettant d'enregistrer deux ou trois dérivations bipolaires (la dernière électrode servant de neutre). Les électrodes sont positionnées sur le thorax, avec un emplacement pouvant varier selon la pathologie suspectée chez le patient.

\subsection{Interprétation de l'ECG}

Lors d'un cycle cardiaque, la dépolarisation et repolarisation du c\{oe}ur se traduisent sur un enregistrement ECG par une serie d'ondes (Figure \ref{ecgondes} : 

\begin{enumerate}
\item L'onde P est une onde positive de faible amplitude et arrondie. Elle correspond à la dépolarisation des oreillettes.
\item Le complexe QRS est un ensemble de trois ondes. Elle correspond à la dépolarisation des ventricules.
\item L'onde T est une onde positive et alongée, correspondant à la repolarisation des ventricules.
\end{enumerate}

En dehors de ces ondes, le signal ECG est normalement plat, et est appelé la ligne isoélectrique.
...
\begin{enumeration}
\item L'intervalle PR correspond au delai entre la dépolarisation auriculaire et ventriculaire. Il donne ainsi la mesure du temps de propagation de la dépolarisation entre les oreilletes et les ventricules.
\item L'intervalle QT est mesuré depuis le début du complexe QRS jusqu'à la fin de l'onde R.
\item Le segment ST est le segment reliant le complexe QRS à l'onde T. Il correspond à la période de dépolarisation ventriculaire et suit la ligne isoélectrique.
\item L'intervalle RR mesure la période cardiaque.
\end{enumeration}

Ces intervalles varient selon l'espèce chez laquelle l'ECG est enregistré. Par exemple, l'intervalle QT chez le cochon est très grand, et le segment ST chez le rat est inexistant (Figure \ref{ecganimaux}.

\begin{figure}[h]
	\centering
	\includegraphics[width=\linewidth]{images/ecganimaux.gif}
	\caption{Enregistrement ECG des voies I, II et III chez un chien, un cochon et un singe. Le chien a une arythmie respiratoire sinusale prononcée, le cochon a un intervalle QT long et le singe une fréquence cardiaque rapide (D'apreès \citet{hamlin05})}
	\label{ecganimaux}
\end{figure}

Lors d'un enregistrement, des artéfacts peuvent apparaître sur le tracé, qui n'ont pas comme origine l'activité électrique cardiaque. Ces artéfacts peuvent rendre plus difficile la lecture des tracés, et altérer l'efficacité des traitements automatiques (Figure \ref{artefactchien}). On peut observer une fluctuation de la ligne de base, qui peut être due à un déplacement des électrodes. Cette fluctuation est généralement corrigée avant chaque traitement automatisé des enregistrements. De même, des artéfacts ponctuels peuvent présenter des variations très importantes et très rapides, occultant les ondes du cycle cardiaque. Ces artéfacts sont plus difficiles à séparer des ondes générées par l'activité cardiaque, car ils partagent les mêmes caractéristiques fréquentielles.


\begin{figure}[h]
	\centering
	\includegraphics[width=\linewidth]{images/artefactchien.png}
	\caption{ (D'apreès \citet{stern13})}
	\label{artefectchien}
\end{figure}
...
La fibrillation atriale (FA) est une anomalie du rythme cardiaque qui est caracterisée par une activation électrique rapide et irrégulière des oreillettes, ce qui induit une contraction désordonnées des oreillettes \citep{falk01}. Par conséquence, la contraction des ventricules est aussi irrégulière et généralement rapide, tant que le n\{oe}ud AV conduit l'impulsion électrique.

La FA est l'arythmie soutenue la plus fréquente, touchant autour de 1 \% de la population générale et 10 \% des personnes de plus de 80 ans \citep{charlemagne11}. Sa principale complication est l'accident vasculaire cérébral \citep{wolf91} dont le risque augmente de 5 fois. Elle est associée à une augmentation du risque de mortalité de 1,5 à 1,9 fois, indépendamment d'autres facteurs de risque \citep{benjamin98}.

Différentes formes cliniques de la FA sont observées, et ont été classées en plusieurs catégories \citep{camm10} :

\begin{description}
\item [La FA paroxystique] se termine spontanément dans les 7 jours ;
\item [La FA persistente] dure plus de 7 jours ou nécessite une cardioversion ;
\item [La FA permanente] est dite lorsque des interventions de controle du rythme ne sont plus envisagées.
\end{description}

La FA progresse généralement de courts et rares épisodes à des épisodes plus longs et fréquents. Il a été montré que près de 9 \% des patients progressent d'une FA paroxystique à une FA permanente dans l'année après le premier diagnostique, et près de 25 \% dans les cinq ans \citep{kerr05}.

Le diagnostique de FA se fait par l'enregistrement d'un ECG (Figure \ref{ecgfa}). La FA est caractérisée sur un tracé ECG par les caractéristiques suivantes :
...
\begin{figure}[h]
	...
	\caption{Enregistrement ECG 12 dérivations présentant une fibrillation atriale.}
	\label{ecgfa}
\end{figure}
...
\begin{figure}[h]
	...
	\caption{Schémas des oreillettes droite et gauche illustrant les différentes hypothèses du mécanisme de la fibrillation atriale (D'après \citet{eckstein08}.}
	\label{ecgfa}
\end{figure}

La génèse d'une arythmie cardiaque dépend de trois facteurs qui composent le triangle de Coumel \citep{farre04} : un substrat arythmogène, un déclencheur et une modification du milieu. Ces trois facteurs sont associés au déclenchement et à la perpétuation de la fibrillation atriale : des anomalités structurelles qui permettent les circuits de réentrée, des foyers ectopiques principalement autour des veines pulmonaires, et un déséquilibre du système nerveux autonome.

Le principal objectif dans le traitement de la fibrillation atriale est de prévenir les complications sévères, principalement les accidents thrombo-emboliques. Le score CHA2DS2VASc a été proposé pour aider à estimer le risque embolique d'un patient \citep{camm10}. Ce score associe un ou deux points à la présence d'un facteur de risque : insuffisance cardiaque, hypertension artérielle, âge, diabète, pathologie vasculaire. Si le score est supérieur à 2, il est recommendé de mettre en place un traitement chronique par un anticoagulant oral. Ensuite, le traitement de la FA fait l'objet de deux stratégies : un controle de la fréquence cardiaque ou un controle du rythme cardiaque. Il n'a pas été montré qu'une stratégie était supérieure à l'autre en termes d'espérence de vie \citep{kong10}. Le controle de la fréquence cardiaque est obtenu principalement par l'utilisation de beta-bloquants, d'antagonistes calciaques ou de la digoxine. Le controle du rythme cardiaque peut être obtenu par plusieurs approches. Certains médicaments bloquant les canaux sodiques, potassiques ou calciques ont un effet antiarythmique. Cependant, l'efficacité de ces produits reste modeste, et ils peuvent avoir des effets pro-arythmiques. Le controle du rythme peut aussi être obtenu par une ablation des veines pulmonaires, mais cette méthode souffre d'un taux élevé de réapparition (25 \% sur 5 ans \citep{shah08}).


\subsection{Modèles animaux de fibrillation atriale}

La première méthode pour obtenir une FA soutenue chez l'animal est l'entrainement électrique atrial. L'entrainement provoque un remodelage atrial en raccourcissant la période réfractaire, et de manière hétérogène. Ce remodelage augmente la vulnérabilité à l'induction de la FA et sa perpétuation \citep{wijffels95}.

L'insuffisance cardiaque, une des causes cliniques principales de FA, a été étudié dans des modèles expérimentaux de chien et de mouton \citep{li99, tanaka07}. Dans ces modèles, l'insuffisance cardiaque est induite par un entrainement du ventricule droit pendant plusieurs semaines. La période réfractaire atriale est inchangée dans ces modèles, mais l'insuffisance cardiaque induit de la fibrose qui entraine des zones de conduction lente et stabilise les circuits de réentrée.

\subsubsection{Modèles muridés de fibrillation atriale}

Les arythmies possédant des circuits de réentrée sont généralement considérées comme restreintes aux grand animaux, car la perpétuation de ces arythmies nécessite une masse cardiaque minimale. En conséquence, la fibrillation atriale est considérée comme impossible chez le rat ou la souris. Cependant, des modèles de tachyarythmies atriales ont été développés chez ces espèces.

Chez le rat, une plus grande susceptibilité aux arythmies atriales a été rapportée sur des c\{oe}urs isolés perfusés de rats agés spontanément hypertendus (de souche SHR) par rapport aux jeunes SHRs et aux controles normotendus \citep{choisy07}. Les SHRs agés ont développé un substrat arythmogène avec une fibrose de l'oreillette gauche et une augmentation de la période réfractaire.
...
\begin{figure}[h]
	...
	\caption{Enregistrements d'ECG d'\{oe}sophage, d'ECG de surface et de pression artérielle chez un rat, après entrainement électrique atrial lors d'une asphyxie. (A) Complexe atrial ; (V) complexe ventriculaire (D'après \citet{haugan04}.}
	\label{ratfa}
\end{figure}

De même, Ono et coll. \citep{ono07} ont rapporté des arythmies atriales après perfusion d'une solution de Tyrode sur des c\{oe}urs isolés. Cette solution altère la recapture des ions calciums par de réticulum sarcoplasmique, ce qui provoque des EAD autour des veines pulmonaires.

Quelques études ont porté sur des modeles transgéniques de souris. Chez la souris knock-out pour le gène Cx40 d'une connexine, des arythmies atriales ont pu être provoquées sur 63 \% des animaux par entrainement trans\{oe}sophagien \citep{hagendorff99}. De même, des souris exprimant l'inhibiteur de l'enzyme de conversion uniquement au niveau cardiaque fait des arythmies atriales de manière spontanée \citep{xiao04}.

\begin{figure}[h]
	\centering
	\includegraphics[width=\linewidth]{images/sourisfa.jpg}
	\caption{Enregistrements ECG télémétriques chez (A) une souris controle et (B) une souris exprimant l'inhibiteur de l'enzyme de conversion uniquement au niveau cardiaque. Le deuxième enregistrement montre des complexes QRS de faible amplitude, aucune onde P visible et des intervalles RR irréguliers (D'après \citet{xiao04}.}
	\label{sourisfa}
\end{figure}
